...
packages <- c("knitr", "ggplot2", "dplyr", "shiny", "scales", "data.table"
)
...
renderPlot({
	...
	area_plot <- ggplot() + geom_area(data = by_age(), aes(x = Age, y = percent, fill=state), position = 'stack', colour = "gray", alpha = 0.5) +
		...
		geom_line(data = c_year(), aes(x=Age, y = Cost/(8*n), colour = "Yearly Cost")) +
		scale_y_continuous(sec.axis = sec_axis(~.*8, name = "Per patient cost", labels = scales::dollar), expand=c(0,0)) +
